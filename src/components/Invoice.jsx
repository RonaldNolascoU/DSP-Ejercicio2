...
const getBaseCost = (size) => {
  ...
  const getIndividualPrice = (size, extraIngredients) => {
    const sizeToPriceMap = {
      'Personal': [1.00, 0.75, 0.50, 0.25],
      'Mediana': [2.00, 1.00, 0.75, 0.50],
      'Grande': [2.50, 2.00, 1.00, 0.75],
    };
...
function Invoice({ data, onBack }) {
  ...
  const { baseCost, additionalCost, total } = calculateCost(size, extraIngredients);

  return (
 <div className="max-w-5xl mx-auto py-16 bg-white">
    {/* Add back icon */}
    <button className="flex items-center text-slate-700 hover:text-slate-600" onClick={onBack}>
    ...
    <div className="p-9">
     <div className="flex w-full">
      <div className="grid grid-cols-4 gap-12">
       ...
       <div className="text-sm font-light text-slate-500">
        <p className="text-sm font-normal text-slate-700">Tipo de Pizza</p>
        <p>{size}</p>
       ...
       <tbody>
        <tr className="border-b border-slate-200">
         <td className="py-4 pl-4 pr-3 text-sm sm:pl-6 md:pl-0">
          <div className="font-medium text-slate-700">Pizza {size}</div>
         </td>
        ...
        {Array(extraIngredients).fill().map((_, index) => (
            <tr className="border-b border-slate-200" key={index}>
                <td className="py-4 pl-4 pr-3 text-sm sm:pl-6 md:pl-0">
                <div className="font-medium text-slate-700">Ingrediente extra #{index + 1}</div>
